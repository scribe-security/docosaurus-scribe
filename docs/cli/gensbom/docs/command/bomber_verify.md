...
```
bomber verify [TARGET] [flags]
```

### Examples

```
  bomber verify <target>
  bomber verify alpine:latest                         verify target against signed attestation
  bomber verify alpine:latest -vv                     show verbose debug information

  Supports the following image sources:
  bomber verify yourrepo/yourimage:tag     defaults to using images from a Docker daemon. If Docker is not present, the image is pulled directly from the registry.

  You can also explicitly specify the scheme to use:
  bomber verify docker:yourrepo/yourimage:tag          explicitly use the Docker daemon
  bomber verify docker-archive:path/to/yourimage.tar   use a tarball from disk for archives created from "docker save"
  bomber verify oci-archive:path/to/yourimage.tar      use a tarball from disk for OCI archives (from Skopeo or otherwise)
  bomber verify dir:path/to/yourproject                read directly from a path on disk (any directory)
  bomber verify registry:yourrepo/yourimage:tag        pull image directly from a registry (no container runtime required)
  bomber verify file:path/to/yourproject/file          read directly from a path on disk (any single file)
